...
💡 *Quick Tip*: Populate your technical stack fully on your profile to maximize compatibility with the jury evaluation systems!`;
  ...
  return (
    <div className="min-h-screen bg-chess-pattern-subtle text-[#0B1E36] font-sans flex flex-col antialiased selection:bg-[#0B1E36]/10 relative">
      ...
      {/* DELL AI ADVISOR FLOATING CONTEXT HUB */}
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
        {/* Glowing trigger button */}
        <button
          onClick={() => setIsAiHubOpen(!isAiHubOpen)}
          className="flex items-center gap-2.5 px-5 py-3.5 bg-gradient-to-r from-[#0076ce] to-[#11253e] hover:from-[#00558f] hover:to-[#0c2340] text-white rounded-full shadow-2xl hover:shadow-[0_0_25px_rgba(0,118,206,0.35)] active:scale-95 transition-all duration-300 border border-white/10 group cursor-pointer"
        >
          <div className="p-1 bg-white/10 rounded-full animate-pulse group-hover:rotate-12 transition-transform">
            <Sparkles className="h-4.5 w-4.5 text-[#C4B495]" />
          </div>
          <span className="text-xs font-black uppercase tracking-widest font-mono">
            {isAiHubOpen ? 'Close AI Advisor' : 'Dell AI Advisor Co-pilot'}
          </span>
        ...
        {isAiHubOpen && (
          <div className="mt-4 w-[92vw] sm:w-[500px] h-[80vh] bg-white/95 backdrop-blur-xl border border-slate-200 shadow-[0_15px_60px_-15px_rgba(0,0,0,0.18)] rounded-2xl flex flex-col overflow-hidden animate-fade-in z-50">
            {/* Header */}
            <div className="p-5 bg-[#0B1E36] text-white flex justify-between items-center border-b border-[#DFD7C7]/20">
              <div className="flex items-center gap-2">
                <Sparkles className="h-5 w-5 text-[#C4B495] animate-pulse" />
                <div className="text-left">
                  <h3 className="text-sm font-black uppercase tracking-wider font-mono text-[#FAF8F5]">Dell Innovation Accelerator</h3>
                  <span className="text-[10px] text-slate-300 font-mono">Secure Enterprise-grade Proxy Server Connection</span>
                </div>
              </div>
              <button
                onClick={() => setIsAiHubOpen(false)}
                className="text-slate-400 hover:text-white transition-colors p-1 font-mono text-sm leading-none"
              >
                ✕
              </button>
            </div>
            ...
            {/* AI Control Tabs */}
            <div className="flex border-b border-slate-100 bg-[#FAF8F5]">
              <button
                onClick={() => setAiActiveSection('audit')}
                className={`flex-1 py-3 text-[10.5px] font-bold uppercase tracking-wider font-mono border-b-2 text-center transition-all ${
                  aiActiveSection === 'audit' ? 'border-[#0076ce] text-[#0076ce] bg-white' : 'border-transparent text-slate-500 hover:text-slate-800'
                }`}
              >
                🏆 Potential Auditor
              </button>
              <button
                ...
                className={`flex-1 py-3 text-[10.5px] font-bold uppercase tracking-wider font-mono border-b-2 text-center transition-all ${
                  aiActiveSection === 'pitch' ? 'border-[#0076ce] text-[#0076ce] bg-white' : 'border-transparent text-slate-500 hover:text-slate-800'
                }`}
              >
                🎙️ Video Pitch Scripter
              </button>
              <button
                onClick={() => setAiActiveSection('duplicates')}
                className={`flex-1 py-3 text-[10.5px] font-bold uppercase tracking-wider font-mono border-b-2 text-center transition-all ${
                  aiActiveSection === 'duplicates' ? 'border-[#0076ce] text-[#0076ce] bg-white' : 'border-transparent text-slate-500 hover:text-slate-800'
                }`}
              >
                🕵️ Fuzzy Audit Dups
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              {/* AUDIT SECTION */}
              {aiActiveSection === 'audit' && (
                <div className="space-y-4">
                  <div className="p-3 bg-sky-50 text-[#0076ce] rounded border border-sky-100/50 text-[11px] leading-relaxed text-left">
                    <strong>Pillar Benchmarking Analyzer:</strong> Score your hackathon idea against Dell Technologies four foundational pillars to find structural pivots and optimization strategies to win the jury votes.
                  </div>

                  <div className="space-y-3 pt-1 text-left">
                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-bold text-slate-500 block font-mono">Project Title</label>
                      <input
                        type="text"
                        value={auditParams.title}
                        onChange={(e) => setAuditParams({ ...auditParams, title: e.target.value })}
                        className="w-full px-3 py-2 border border-slate-200 rounded text-xs text-slate-900 bg-white"
                      />
                    </div>

                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-bold text-slate-500 block font-mono">Elevator Tagline Spark</label>
                      <input
                        type="text"
                        value={auditParams.tagline}
                        onChange={(e) => setAuditParams({ ...auditParams, tagline: e.target.value })}
                        className="w-full px-3 py-2 border border-slate-200 rounded text-xs text-slate-900 bg-white"
                      />
                    </div>

                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-bold text-slate-500 block font-mono">Operational Technical Stack</label>
                      <input
                        type="text"
                        value={auditParams.techStack}
                        onChange={(e) => setAuditParams({ ...auditParams, techStack: e.target.value })}
                        className="w-full px-3 py-2 border border-slate-200 rounded font-mono text-xs text-slate-900 bg-white"
                      />
                    </div>

                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-bold text-slate-500 block font-mono">Detailed Functional Description</label>
                      <textarea
                        rows={3}
                        value={auditParams.description}
                        onChange={(e) => setAuditParams({ ...auditParams, description: e.target.value })}
                        className="w-full px-3 py-2 border border-slate-200 rounded text-xs text-slate-900 bg-white resize-none"
                      />
                    </div>
                    ...
                    <button
                      ...
                      disabled={isAuditing}
                      className="w-full py-2.5 bg-[#0B1E36] hover:bg-[#18304F] text-white rounded font-mono font-bold text-xs uppercase tracking-wider hover:shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 border-none"
                    >
                      {isAuditing ? (
                        <>
                          <RefreshCw className="h-4 w-4 animate-spin" />
                          <span>Auditing Pitch Matrices...</span>
                        </>
                      ) : (
                        <>
                          <Award className="h-4 w-4 text-[#C4B495]" />
                          <span>Audit For First Place (Gemini 3.5)</span>
                        </>
                      )}
                    </button>
                  </div>
                  ...
                  {/* Audit Outputs */}
                  {auditResult && (
                    <div className="p-4 bg-slate-50 border border-slate-250 rounded-lg text-left shadow-inner mt-4">
              ...
              {/* PITCH SECTION */}
              {aiActiveSection === 'pitch' && (
                <div className="space-y-4">
                  <div className="p-3 bg-amber-50 text-amber-800 rounded border border-amber-100 text-[11px] leading-relaxed text-left">
                    <strong>60-Second Video Script writer:</strong> Input your project parameters above and select your target judging panel to script a high-impact, professional speech featuring strategic cues.
                  </div>

                  <div className="space-y-3 pt-1 text-left">
                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-bold text-slate-500 block font-mono">Target Judging Panel</label>
                      <select
                        value={pitchAudience}
                        onChange={(e: any) => setPitchAudience(e.target.value)}
                        className="w-full px-3 py-2 border border-slate-200 rounded text-xs text-slate-900 bg-white"
                      >
                        <option value="Dell Enterprise Venture Capital">Dell Enterprise Venture Capital (ROI & Scaling focus)</option>
                        <option value="Technical Faculty Director">Technical Faculty Director (Rigor, Code coverage & Speed focus)</option>
                        <option value="Green Computing Council">Green Computing Council (GreenOps & Thermal metrics focus)</option>
                      </select>
                    </div>
                    ...
                    >
                      {isGeneratingPitch ? (
                        <>
                          <RefreshCw className="h-4 w-4 animate-spin" />
                          <span>Generating Cue Cards...</span>
                        </>
                      ) : (
                        <>
                          <Zap className="h-4 w-4 text-[#C4B495]" />
                          <span>Compile Video Pitch Script</span>
                        </>
                      )}
                    </button>
                  </div>
                  ...
                  {/* Pitch Outputs */}
                  {pitchResult && (
                    <div className="p-4 bg-[#FAF8F5] border border-[#DFD7C7] rounded-lg text-left shadow-inner mt-4">
              ...
              {/* DE-DUPLICATE FUZZY CHECKER SECTION */}
              {aiActiveSection === 'duplicates' && (
                <div className="space-y-4">
                  <div className="p-3 bg-emerald-50 text-emerald-800 rounded border border-emerald-100 text-[11px] leading-relaxed text-left">
                    <strong>Fuzzy Duplicate Scan:</strong> Proactively check if a new registrant overlaps semantically with current entries, preventing intellectual theft or double-allocations dynamically.
                  </div>

                  <div className="space-y-3 pt-1 text-left">
                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-bold text-slate-500 block font-mono">Simulated Challenger Account First Name</label>
                      <input
                        type="text"
                        value={dupScanNewProfile.firstName}
                        onChange={(e) => setDupScanNewProfile({ ...dupScanNewProfile, firstName: e.target.value })}
                        className="w-full px-3 py-2 border border-slate-200 rounded text-xs text-slate-900 bg-white"
                      />
                    </div>

                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-bold text-slate-500 block font-mono">Simulated Challenger Account Last Name</label>
                      <input
                        type="text"
                        value={dupScanNewProfile.lastName}
                        onChange={(e) => setDupScanNewProfile({ ...dupScanNewProfile, lastName: e.target.value })}
                        className="w-full px-3 py-2 border border-slate-200 rounded text-xs text-slate-900 bg-white"
                      />
                    </div>

                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-bold text-slate-500 block font-mono">Challenger Email</label>
                      <input
                        type="email"
                        value={dupScanNewProfile.email}
                        onChange={(e) => setDupScanNewProfile({ ...dupScanNewProfile, email: e.target.value })}
                        className="w-full px-3 py-2 border border-slate-200 rounded text-xs text-slate-900 bg-white"
                      />
                    </div>

                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-bold text-slate-500 block font-mono">Challenger Technical Biography</label>
                      <textarea
                        rows={2}
                        value={dupScanNewProfile.personalBio}
                        onChange={(e) => setDupScanNewProfile({ ...dupScanNewProfile, personalBio: e.target.value })}
                        className="w-full px-2.5 py-2 border border-slate-200 rounded text-xs text-slate-900 bg-white resize-none"
                      />
                    </div>
                    ...
                    >
                      {isDupScanning ? (
                        <>
                          <RefreshCw className="h-4 w-4 animate-spin" />
                          <span>Deploying Fuzzy Scan Pipelines...</span>
                        </>
                      ) : (
                        <>
                          <Search className="h-4 w-4 text-[#C4B495]" />
                          <span>Scan For Intellectual Duplicity</span>
                        </>
                      )}
                    </button>
                  </div>
                  ...
                  {/* Scan Outputs */}
                  {dupScanResult && (
                    <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg text-left shadow-inner mt-4 space-y-2">
                      <div className="flex justify-between items-center pb-2 border-b border-slate-200">
                        <strong className="text-xs uppercase tracking-wide font-mono text-slate-600">Scan Assessment Result:</strong>
                        <span className={`px-2.5 py-0.5 rounded text-[10px] font-mono font-extrabold uppercase ${
                          dupScanResult.duplicateDetected ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'
                        }`}>
                          {dupScanResult.duplicateDetected ? '🚨 Flagged Conflict' : '✓ Distinct Registrant'}
                        </span>
                      </div>
                      <div className="text-xs text-slate-700">
                        <div className="text-[11px] font-mono text-slate-400 mb-1">PROBABILITY CONFIDENCE WEIGHT: {dupScanResult.confidenceScore}%</div>
                        <p className="font-medium">{dupScanResult.reason}</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {/* Footer */}
            <div className="p-4 bg-[#FAF8F5] border-t border-slate-100 flex justify-between items-center text-[10px] font-mono text-slate-400">
              <span>Dell Telemetry Core IP Sync v3.5</span>
